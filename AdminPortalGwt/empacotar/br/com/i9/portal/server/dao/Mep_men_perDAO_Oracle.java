...
public class Mep_men_perDAO_Oracle extends ObjectDAO implements IMep_men_perDAO {

    public Mep_men_perDAO_Oracle(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public List<Mep_men_perTGWT> getByPer_perfil(Mep_men_perTGWT mep_men_perT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.mep_men_per where per_perfil.per_nr_id=?  ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, mep_men_perT.getPer_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
