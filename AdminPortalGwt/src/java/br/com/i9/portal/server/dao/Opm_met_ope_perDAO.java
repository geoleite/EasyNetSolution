...
public class Opm_met_ope_perDAO extends ObjectDAOClusterAdminPortal implements IOpm_met_ope_perDAO {

    public Opm_met_ope_perDAO(DAOFactoryCluster dao) throws Exception {
        //setDAOFactory(dao);
        //con = dao.create();
    ...
    public void update(Opm_met_ope_perTGWT opm_met_ope_perT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "update portal.opm_met_ope_per set - where  per_nr_id=? and ope_nr_id=? and sis_nr_id=? and met_nr_id=?";
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, opm_met_ope_perT.getPer_nr_id());
            pStmt.setObject(2, opm_met_ope_perT.getOpe_nr_id());
            pStmt.setObject(3, opm_met_ope_perT.getSis_nr_id());
    ...
    public List<Opm_met_ope_perTGWT> getByMet_nr_id(Opm_met_ope_perTGWT opm_met_ope_perT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.opm_met_ope_per where  met_nr_id = ? ";
            pStmt = createPrepareStatment(sql);
    ...
    public List<Opm_met_ope_perTGWT> getByPer_perfil(Opm_met_ope_perTGWT opm_met_ope_perT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.opm_met_ope_per where per_perfil.per_nr_id=?  ";
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, opm_met_ope_perT.getPer_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Opm_met_ope_perTGWT> getByOpe_operacao(Opm_met_ope_perTGWT opm_met_ope_perT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.opm_met_ope_per where ope_operacao.ope_nr_id=? and ope_operacao.sis_nr_id=?  ";
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, opm_met_ope_perT.getOpe_nr_id());
    ...
    public List<Opm_met_ope_perTGWT> getByMet_metodo(Opm_met_ope_perTGWT opm_met_ope_perT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.opm_met_ope_per where met_metodo.met_nr_id=?  ";
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, opm_met_ope_perT.getMet_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
