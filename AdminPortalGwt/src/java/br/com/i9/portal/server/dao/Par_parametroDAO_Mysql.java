...
public class Par_parametroDAO_Mysql extends ObjectDAO implements IPar_parametroDAO {

    public Par_parametroDAO_Mysql(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Par_parametroTGWT par_parametroT) throws Exception {
        ...
        try {
            String sql = "insert into par_parametro  ( sis_nr_id, par_tx_nome, par_tx_valor, par_tx_secreto) values ( ? , ? , ?, ? )";
    ...
    public void update(Par_parametroTGWT par_parametroT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "update par_parametro set  par_tx_valor=?, par_tx_nome=?, sis_nr_id=?, par_tx_secreto=?   where par_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public void delete(Par_parametroTGWT par_parametroT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "delete from par_parametro where par_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, par_parametroT.getSis_nr_id());
    ...
    public List<Par_parametroTGWT> resultSetToObjectTransfer(ResultSet rs) throws Exception {
        ...
        while (rs.next()) {
            ...
            par_parametroT.setSis_nr_id(rs.getInt("par_nr_id"));
    ...
    public List<Par_parametroTGWT> getAll() throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from par_parametro";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Par_parametroTGWT> getByPK(Par_parametroTGWT par_parametroT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from par_parametro where  par_nr_id=?";
            pStmt = con.prepareStatement(sql);
            //pStmt.setObject(1, par_parametroT.getSis_nr_id());
    ...
    public List<Par_parametroTGWT> getByPar_tx_nome(Par_parametroTGWT par_parametroT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from par_parametro where  Upper(par_tx_nome) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Par_parametroTGWT> getByPar_tx_valor(Par_parametroTGWT par_parametroT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from par_parametro where  Upper(par_tx_valor) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, '%' + par_parametroT.getPar_tx_valor() + '%');
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
