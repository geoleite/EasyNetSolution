...
public class Ope_operacaoDAO_Oracle extends ObjectDAO implements IOpe_operacaoDAO {

    public Ope_operacaoDAO_Oracle(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        try {
            String sql = "insert into usr_portal.ope_operacao  ( sis_nr_id, ope_tx_nome, ope_tx_status, ope_tx_url, ope_tx_descricao, ope_tx_classe) values ( ? , ? , ? , ? , ? , ? )";
    ...
    public void update(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "update usr_portal.ope_operacao set  ope_tx_nome=?, ope_tx_status=?, ope_tx_url=?, ope_tx_descricao=?, ope_tx_classe=?  where  ope_nr_id=? and sis_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public void delete(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "delete from usr_portal.ope_operacao where  ope_nr_id=? and sis_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoTGWT> getAll() throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoTGWT> getByPK(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  ope_nr_id=? and sis_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, ope_operacaoT.getOpe_nr_id());
            pStmt.setObject(2, ope_operacaoT.getSis_nr_id());
            rs = pStmt.executeQuery();
    ...
    public List<Ope_operacaoTGWT> getByOpe_nr_id(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  ope_nr_id = ? ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, ope_operacaoT.getOpe_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Ope_operacaoTGWT> getBySis_nr_id(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  sis_nr_id = ?  order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, ope_operacaoT.getSis_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Ope_operacaoTGWT> getByPerfil(Per_perfilTGWT perT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao ope where ope.ope_nr_id in (select distinct ope_nr_id from usr_portal.opm_met_ope_per where per_nr_id=?)  order by ope_tx_nome ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoTGWT> getByNotPerfil(Per_perfilTGWT perT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao ope where ope.ope_nr_id not in (select distinct ope_nr_id from usr_portal.opm_met_ope_per where per_nr_id=?)  order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, perT.getPer_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Ope_operacaoTGWT> getByOpe_tx_nome(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  Upper(ope_tx_nome) like Upper(?)  order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, '%' + ope_operacaoT.getOpe_tx_nome() + '%');
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Ope_operacaoTGWT> getByOpe_tx_status(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  Upper(ope_tx_status) like Upper(?)  order by ope_tx_nome ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoTGWT> getByOpe_tx_url(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  Upper(ope_tx_url) like Upper(?)  order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoTGWT> getByOpe_tx_descricao(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  Upper(ope_tx_descricao) like Upper(?)  order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoTGWT> getByOpe_tx_classe(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where  Upper(ope_tx_classe) like Upper(?)  order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoTGWT> getBySis_sistema(Ope_operacaoTGWT ope_operacaoT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from usr_portal.ope_operacao where sis_sistema.sis_nr_id=?  ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, ope_operacaoT.getSis_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
