...
public class Int_interfaceDAO_Mysql extends ObjectDAO implements IInt_interfaceDAO{

    public Int_interfaceDAO_Mysql(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Int_interfaceT int_interfaceT) throws Exception {
        ...
        try {
            String sql = "insert into int_interface  ( int_tx_nome) values ( ? )";
    ...
    public void update(Int_interfaceT int_interfaceT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "update int_interface set  int_tx_nome=?  where  int_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, int_interfaceT.getInt_tx_nome());
    ...
    public void delete(Int_interfaceT int_interfaceT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "delete from int_interface where  int_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Int_interfaceT> getAll() throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from int_interface order by int_tx_nome";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Int_interfaceT> getByPK(Int_interfaceT int_interfaceT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from int_interface where  int_nr_id=? order by int_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, int_interfaceT.getInt_nr_id());
            rs = pStmt.executeQuery();
    ...
    public List<Int_interfaceT> getByInt_tx_nome(Int_interfaceT int_interfaceT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from int_interface where  Upper(int_tx_nome) like Upper(?) order by int_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, '%' + int_interfaceT.getInt_tx_nome() + '%');
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
