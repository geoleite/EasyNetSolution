...
public class Par_parametroDAO extends ObjectDAOClusterEasyPortal implements IPar_parametroDAO{
    ...
    public void update(Par_parametroT par_parametroT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "update portal.par_parametro set  par_tx_valor=?, sis_nr_id=?, par_tx_nome=?  where  par_nr_id=?";
            //pStmt = con.prepareStatement(sql);
            pStmt = createPrepareStatment(sql);
    ...
    public List<Par_parametroT> getAll() throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.par_parametro order by sis_nr_id, par_tx_nome";
            //pStmt = con.prepareStatement(sql);
            pStmt = createPrepareStatment(sql);
    ...
    public Par_parametroT getByPK(Par_parametroT par_parametroT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.par_parametro where  sis_nr_id=? and upper(par_tx_nome)= upper(?) ";
            //pStmt = con.prepareStatement(sql);
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, par_parametroT.getSis_nr_id());
            pStmt.setObject(2, par_parametroT.getPar_tx_nome());
    ...
    public List<Par_parametroT> getByPar_tx_nome(Par_parametroT par_parametroT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.par_parametro where  Upper(par_tx_nome) like Upper(?) order by par_tx_nome";
            //pStmt = con.prepareStatement(sql);
            pStmt = createPrepareStatment(sql);
    ...
    public List<Par_parametroT> getByPar_tx_valor(Par_parametroT par_parametroT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from portal.par_parametro where  Upper(par_tx_valor) like Upper(?)  order by par_tx_nome";
            //pStmt = con.prepareStatement(sql);
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, '%' + par_parametroT.getPar_tx_valor() + '%');
            rs = pStmt.executeQuery();
