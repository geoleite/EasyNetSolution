...
public class Ope_operacaoDAO_Oracle extends ObjectDAO implements IOpe_operacaoDAO{
    ...
    public List<Ope_operacaoT> getById(Ope_operacaoT ope_operacaoT) throws Exception {
        try {
            String sql = "select * from usr_portal.ope_operacao where  ope_nr_id=? and sis_nr_id=? order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, ope_operacaoT.getOpe_nr_id());
            pStmt.setObject(2, ope_operacaoT.getSis_nr_id());
            rs = pStmt.executeQuery();
    ...
    public List<Ope_operacaoT> getOperacoesPerfil(Per_perfilT per_perfilT) throws Exception {
        try {
            String sql = "select * from usr_portal.ope_operacao where ope_nr_id in (select ope_nr_id from usr_portal.opm_met_ope_per op where op.per_nr_id=?) order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Ope_operacaoT> getOperacoesNaoPerfil(Per_perfilT per_perfilT) throws Exception {
        try {
            String sql = "select * from usr_portal.ope_operacao where ope_nr_id not in (select ope_nr_id from usr_portal.opm_met_ope_per where per_nr_id=?) order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, per_perfilT.getPer_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Ope_operacaoT> getByOpe_nr_id(Ope_operacaoT ope_operacaoT) throws Exception {
        try {
            String sql = "select * from usr_portal.ope_operacao where  ope_nr_id = ? order by ope_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, ope_operacaoT.getOpe_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Ope_operacaoT> getBySis_nr_id(Ope_operacaoT ope_operacaoT) throws Exception {
        try {
            String sql = "select * from usr_portal.ope_operacao where  sis_nr_id = ? order by ope_tx_nome, ope_tx_descricao ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, ope_operacaoT.getSis_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public Sis_sistemaT getBySis_sistemaT(Ope_operacaoT ope_operacaoT) throws Exception {
        ISis_sistemaDAO dao = new Sis_sistemaDAO_Oracle(getDAOFactory());
