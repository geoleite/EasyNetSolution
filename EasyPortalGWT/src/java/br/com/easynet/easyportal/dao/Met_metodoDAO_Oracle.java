...
public class Met_metodoDAO_Oracle extends ObjectDAO implements IMet_metodoDAO {

    private ResultSet rs = null;
    private PreparedStatement pStmt = null;

    public Met_metodoDAO_Oracle(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "insert into usr_portal.met_metodo  ( ope_nr_id, met_tx_metodo, met_tx_status, sis_nr_id, met_tx_descricao) values ( ? , ? , ? , ?, ?  )";
    ...
    public void update(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "update usr_portal.met_metodo set  ope_nr_id=?, met_tx_metodo=?, met_tx_status=?, sis_nr_id=?, met_tx_descricao=?  where  met_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, met_metodoT.getOpe_nr_id());
            pStmt.setObject(2, met_metodoT.getMet_tx_metodo());
            pStmt.setObject(3, met_metodoT.getMet_tx_status());
    ...
    public void delete(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "delete from usr_portal.met_metodo where  met_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Met_metodoT> getAllMetodosPerfilSistema(Per_perfilT per_perfilT, Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "select * from usr_portal.met_metodo met where  met.sis_nr_id = ? and met.met_nr_id in (select met_nr_id from usr_portal.opm_met_ope_per opm where opm.sis_nr_id=? and opm.per_nr_id=?) order by met.met_tx_metodo";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Met_metodoT> getAll() throws Exception {
        try {
            String sql = "select * from usr_portal.met_metodo order by met_tx_metodo";
            pStmt = con.prepareStatement(sql);
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
        } catch (Exception e) {
    ...
    public List<Met_metodoT> getById(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "select * from usr_portal.met_metodo where  met_nr_id=? order by met_tx_metodo";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, met_metodoT.getMet_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Met_metodoT> getByMetodosNaoPerfil(Per_perfilT per_perfilT, Met_metodoT met_metodoT) throws Exception {
        try {
            ...
            String sql = "select * from usr_portal.met_metodo met where  met.sis_nr_id= ? and met.ope_nr_id=? and met.met_nr_id not in (select met_nr_id from usr_portal.opm_met_ope_per opm where opm.sis_nr_id=? and opm.ope_nr_id=? and opm.per_nr_id=?) order by met.met_tx_metodo";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, met_metodoT.getSis_nr_id());
    ...
    public List<Met_metodoT> getByMetodosPerfil(Per_perfilT per_perfilT, Met_metodoT met_metodoT) throws Exception {
        try {
            ...
            String sql = "select * from usr_portal.met_metodo met where  met.sis_nr_id= ? and met.ope_nr_id=? and met.met_nr_id in (select met_nr_id from usr_portal.opm_met_ope_per opm where opm.sis_nr_id=? and opm.ope_nr_id=? and opm.per_nr_id=?) order by met.met_tx_metodo";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, met_metodoT.getSis_nr_id());
            pStmt.setObject(2, met_metodoT.getOpe_nr_id());
            pStmt.setObject(3, met_metodoT.getSis_nr_id());
    ...
    public List<Met_metodoT> getByOpe_nr_id(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "select * from usr_portal.met_metodo where  ope_nr_id = ? order by met_tx_metodo";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, met_metodoT.getOpe_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Met_metodoT> getByMet_tx_metodo(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "select * from usr_portal.met_metodo where  Upper(met_tx_metodo) like Upper(?) order by met_tx_metodo";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Met_metodoT> getByMet_tx_status(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "select * from usr_portal.met_metodo where  Upper(met_tx_status) like Upper(?) order by met_tx_metodo";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Met_metodoT> getBySis_nr_id(Met_metodoT met_metodoT) throws Exception {
        try {
            String sql = "select * from usr_portal.met_metodo where  sis_nr_id = ? order by met_tx_metodo";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, met_metodoT.getSis_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
