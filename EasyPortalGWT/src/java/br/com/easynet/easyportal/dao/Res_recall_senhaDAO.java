...
public class Res_recall_senhaDAO extends ObjectDAOClusterEasyPortal implements IRes_recall_senhaDAO {
    ...
    public void update(Res_recall_senhaT res_recall_senhaT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "update portal.res_recall_senha set  res_tx_pergunta=?, res_tx_resposta=? where  usu_nr_id=?";
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, res_recall_senhaT.getRes_tx_pergunta());
            pStmt.setObject(2, res_recall_senhaT.getRes_tx_resposta());
            pStmt.setObject(3, res_recall_senhaT.getUsu_nr_id());
            pStmt.execute();
        } catch (Exception e) {
            throw e;
        } finally {
    ...
    public List<Res_recall_senhaT> getByLoginEmail(String login, String email) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select res.* from portal.res_recall_senha res, portal.usu_usuario usu where  usu.usu_tx_login=? and usu.usu_tx_email=? and usu.usu_nr_id=res.usu_nr_id";
            pStmt = createPrepareStatment(sql);
