...
public class Op_ope_perDAO_Oracle extends ObjectDAO implements IOp_ope_perDAO{

    private ResultSet rs = null;
    private PreparedStatement pStmt = null;

    public Op_ope_perDAO_Oracle(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public List<Op_ope_perT> getByPer_perfil(Op_ope_perT op_ope_perT) throws Exception {
        try {
            String sql = "select * from usr_portal.op_ope_per where per_perfil.per_nr_id=?  ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, op_ope_perT.getPer_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Op_ope_perT> getByOpe_operacao(Op_ope_perT op_ope_perT) throws Exception {
        try {
            String sql = "select * from usr_portal.op_ope_per where ope_operacao.ope_nr_id=? and ope_operacao.sis_nr_id=?  ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, op_ope_perT.getOpe_nr_id());
