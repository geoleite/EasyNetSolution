...
public class Pi_per_intDAO_Mysql extends ObjectDAO implements IPi_per_intDAO {

    public Pi_per_intDAO_Mysql(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Pi_per_intT pi_per_intT) throws Exception {
        ...
        try {
            String sql = "insert into pi_per_int  ( int_nr_id, per_nr_id) values ( ? , ? )";
    ...
    public void update(Pi_per_intT pi_per_intT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "update pi_per_int set - where  int_nr_id=? and per_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, pi_per_intT.getInt_nr_id());
            pStmt.setObject(2, pi_per_intT.getPer_nr_id());
            pStmt.execute();
    ...
    public void delete(Pi_per_intT pi_per_intT) throws Exception {
        PreparedStatement pStmt = null;
        try {
            String sql = "delete from pi_per_int where  int_nr_id=? and per_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, pi_per_intT.getInt_nr_id());
            pStmt.setObject(2, pi_per_intT.getPer_nr_id());
            pStmt.execute();
    ...
    public List<Pi_per_intT> getAll() throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from pi_per_int";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Pi_per_intT> getByPK(Pi_per_intT pi_per_intT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from pi_per_int where  int_nr_id=? and per_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, pi_per_intT.getInt_nr_id());
            pStmt.setObject(2, pi_per_intT.getPer_nr_id());
            rs = pStmt.executeQuery();
    ...
    public List<Pi_per_intT> getByInt_nr_id(Pi_per_intT pi_per_intT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from pi_per_int where  int_nr_id = ? ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, pi_per_intT.getInt_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Pi_per_intT> getByPer_nr_id(Pi_per_intT pi_per_intT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from pi_per_int where  per_nr_id = ? ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Pi_per_intT> getByInt_interface(Pi_per_intT pi_per_intT) throws Exception {
        ...
        ResultSet rs = null;
        try {
            String sql = "select * from pi_per_int where int_interface.int_nr_id=?  ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, pi_per_intT.getInt_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
