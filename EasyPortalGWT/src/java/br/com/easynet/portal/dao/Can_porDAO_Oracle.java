...
public class Can_porDAO_Oracle extends ObjectDAO implements ICan_porDAO {

    private ResultSet rs = null;
    private PreparedStatement pStmt = null;

    public Can_porDAO_Oracle(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Can_porT can_porT) throws Exception {
        try {
            String sql = "insert into usr_portal.can_por  ( por_nr_id, can_nr_id, cp_nr_ordem) values ( ? , ? , ? )";
    ...
    public void update(Can_porT can_porT) throws Exception {
        try {
            String sql = "update usr_portal.can_por set  cp_nr_ordem=?  where  por_nr_id=? and can_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public void delete(Can_porT can_porT) throws Exception {
        try {
            String sql = "delete from usr_portal.can_por where  por_nr_id=? and can_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, can_porT.getPor_nr_id());
            pStmt.setObject(2, can_porT.getCan_nr_id());
            pStmt.execute();
    ...
    public List<Can_porT> getAllPortal(Can_porT cpT) throws Exception {
        try {
            String sql = "select * from usr_portal.can_por where por_nr_id=? order by can_nr_id";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_porT> getById(Can_porT can_porT) throws Exception {
        try {
            String sql = "select * from usr_portal.can_por where  por_nr_id=? and can_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, can_porT.getPor_nr_id());
            pStmt.setObject(2, can_porT.getCan_nr_id());
            rs = pStmt.executeQuery();
    ...
    public List<Can_porT> getByCp_nr_ordem(Can_porT can_porT) throws Exception {
        try {
            String sql = "select * from usr_portal.can_por where  cp_nr_ordem = ? ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, can_porT.getCp_nr_ordem());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
