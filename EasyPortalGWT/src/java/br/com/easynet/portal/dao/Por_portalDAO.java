...
public class Por_portalDAO extends ObjectDAO implements IPor_portalDAO{

    private ResultSet rs = null;
    private PreparedStatement pStmt = null;

    public Por_portalDAO(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Por_portalT por_portalT) throws Exception {
        try {
            String sql = "insert into canais.por_portal  ( por_tx_nome, por_tx_status, por_nr_colunas) values ( ? , ? , ? )";
    ...
    public void update(Por_portalT por_portalT) throws Exception {
        try {
            String sql = "update canais.por_portal set  por_tx_nome=?, por_tx_status=?, por_nr_colunas=?  where  por_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, por_portalT.getPor_tx_nome());
            pStmt.setObject(2, por_portalT.getPor_tx_status());
            pStmt.setObject(3, por_portalT.getPor_nr_colunas());
    ...
    public void delete(Por_portalT por_portalT) throws Exception {
        try {
            String sql = "delete from canais.por_portal where  por_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Por_portalT> getAll() throws Exception {
        try {
            String sql = "select * from canais.por_portal";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Por_portalT> getById(Por_portalT por_portalT) throws Exception {
        try {
            String sql = "select * from canais.por_portal where  por_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, por_portalT.getPor_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Por_portalT> getByIdUsuario(Usu_usuarioT usu_usuarioT) throws Exception {
        try {
            String sql = "select * from canais.por_portal por, canais.por_usu pu where pu.usu_nr_id=? and pu.por_nr_id=por.por_nr_id";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Por_portalT> getByPor_tx_nome(Por_portalT por_portalT) throws Exception {
        try {
            String sql = "select * from canais.por_portal where  Upper(por_tx_nome) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, '%' + por_portalT.getPor_tx_nome() + '%');
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Por_portalT> getByPor_tx_status(Por_portalT por_portalT) throws Exception {
        try {
            String sql = "select * from canais.por_portal where  Upper(por_tx_status) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Por_portalT> getByPor_nr_colunas(Por_portalT por_portalT) throws Exception {
        try {
            String sql = "select * from canais.por_portal where  por_nr_colunas = ? ";
            pStmt = con.prepareStatement(sql);
