...
public class Can_canalDAO extends ObjectDAO implements ICan_canalDAO{

    private ResultSet rs = null;
    private PreparedStatement pStmt = null;

    public Can_canalDAO(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "insert into canais.can_canal  ( can_tx_nome, can_tx_descricao, can_tx_url, can_tx_status, can_tx_border, can_tx_estado, can_tx_iframe) values ( ? , ? , ? , ? , ? , ? , ? )";
    ...
    public void update(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "update canais.can_canal set  can_tx_nome=?, can_tx_descricao=?, can_tx_url=?, can_tx_status=?, can_tx_border=?, can_tx_estado=?, can_tx_iframe=?  where  can_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, can_canalT.getCan_tx_nome());
            pStmt.setObject(2, can_canalT.getCan_tx_descricao());
            pStmt.setObject(3, can_canalT.getCan_tx_url());
    ...
    public void delete(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "delete from canais.can_canal where  can_nr_id=?";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getAll() throws Exception {
        try {
            String sql = "select * from canais.can_canal";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getAllPortal(Por_portalT porT) throws Exception {
        try {
            String sql = "select * from canais.can_canal can where can.can_nr_id in (select can_nr_id from canais.can_por where por_nr_id=?) order by can.can_tx_nome";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getAllNaoPortal(Por_portalT porT) throws Exception {
        try {
            String sql = "select * from canais.can_canal can where can.can_nr_id not in (select can_nr_id from canais.can_por where por_nr_id=?) order by can.can_tx_nome";
            pStmt = con.prepareStatement(sql);
            pStmt.setInt(1, porT.getPor_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Can_canalT> getById(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  can_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, can_canalT.getCan_nr_id());
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Can_canalT> getByPortal(Por_portalT portalT) throws Exception {
        try {
            String sql = "select can.* from canais.can_canal can, canais.can_por cp where  cp.por_nr_id=? and cp.can_nr_id=can.can_nr_id and can.can_tx_status='A' and can.can_tx_estado <>'CLO' order by cp.cp_nr_ordem";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getByCan_tx_nome(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  Upper(can_tx_nome) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, '%' + can_canalT.getCan_tx_nome() + '%');
            rs = pStmt.executeQuery();
            return resultSetToObjectTransfer(rs);
    ...
    public List<Can_canalT> getByCan_tx_descricao(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  Upper(can_tx_descricao) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getByCan_tx_url(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  Upper(can_tx_url) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getByCan_tx_status(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  Upper(can_tx_status) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getByCan_tx_border(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  Upper(can_tx_border) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getByCan_tx_estado(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  Upper(can_tx_estado) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Can_canalT> getByCan_tx_iframe(Can_canalT can_canalT) throws Exception {
        try {
            String sql = "select * from canais.can_canal where  Upper(can_tx_iframe) like Upper(?) ";
            pStmt = con.prepareStatement(sql);
