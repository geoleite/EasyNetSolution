...
public class Por_usuDAO extends ObjectDAO implements IPor_usuDAO{

    private ResultSet rs = null;
    private PreparedStatement pStmt = null;

    public Por_usuDAO(DAOFactory dao) throws Exception {
        setDAOFactory(dao);
        con = dao.create();
    }
    ...
    public void insert(Por_usuT por_usuT) throws Exception {
        try {
            String sql = "insert into canais.por_usu  ( usu_nr_id, por_nr_id) values ( ? , ? )";
    ...
    public void update(Por_usuT por_usuT) throws Exception {
        try {
            String sql = "update canais.por_usu set - where  usu_nr_id=? and por_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, por_usuT.getUsu_nr_id());
            pStmt.setObject(2, por_usuT.getPor_nr_id());
            pStmt.execute();
    ...
    public void delete(Por_usuT por_usuT) throws Exception {
        String sql = "delete from canais.por_usu where  usu_nr_id=? and por_nr_id=?";
    ...
    public List<Por_usuT> getAll() throws Exception {
        try {
            String sql = "select * from canais.por_usu";
            pStmt = con.prepareStatement(sql);
    ...
    public List<Por_usuT> getById(Por_usuT por_usuT) throws Exception {
        try {
            String sql = "select * from canais.por_usu where  usu_nr_id=? and por_nr_id=?";
            pStmt = con.prepareStatement(sql);
            pStmt.setObject(1, por_usuT.getUsu_nr_id());
            pStmt.setObject(2, por_usuT.getPor_nr_id());
            rs = pStmt.executeQuery();
