...
public class LogDAO_Oracle extends ObjectDAOClusterEasyPortal implements ILogDAO {

    private ResultSet rs = null;
    private PreparedStatement pStmt = null;

    public LogDAO_Oracle(DAOFactoryCluster dao) throws Exception {
        //setDAOFactory(dao);
        //con = dao.create();
    ...
    public void insert(LogT logT) throws Exception {
        try {
            String sql = "insert into usr_portal.log  ( log_tx_sistema, log_tx_classe, log_tx_metodo, log_dt_datahora, log_tx_usuario, log_tx_status, log_tx_ip, log_tx_parametros) values ( ? , ? , ? , ? , ? , ?, ?, ?)";
    ...
    public void update(LogT logT) throws Exception {
        try {
            String sql = "update usr_portal.log set  log_tx_sistema=?, log_tx_classe=?, log_tx_metodo=?, log_dt_datahora=?, log_tx_usuario=?  where  log_nr_id=?";
            //pStmt = con.prepareStatement(sql);
            pStmt = createPrepareStatment(sql);
            pStmt.setObject(1, logT.getLog_tx_sistema());
            pStmt.setObject(2, logT.getLog_tx_classe());
